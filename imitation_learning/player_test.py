...
def test_agent(agent, agent_ids, num_games):
    ...
    for i in range(num_games):
        ...
        players[0].takes_random_actions = False
...
def get_player_action(valid_actions):
    print('Actions: 0-fold, 1-check, 2-call, 3-2BBraise, 4-4BBraise, 5-allin')
    action = int(input(f'choose {", ".join(valid_actions)}\n'))
    return action
        ...
        if not p.folded and not p.is_all_in:
            ...
            if must_go_all_in or all_other_players_all_in_or_folded:
                ...
                if action == 0:
                    ...
                    amount = p.call(pot.main_pot_cutoff + pot.side_pot_cutoff)
            ...
            # if not p.takes_random_actions:
                # print(f'{action =}')
                # print(p.current_bet, current_highest_bet, p.is_all_in, p.folded, p.actions_taken)
            if p.num_actions_this_episode > 100:
                print('SOMETHING WNT AWRY breaking')
                break
            p.num_actions_this_episode += 1
            # input('q')
            # if not p.takes_random_actions:
            #     with open(save_expert_act_file, 'a') as f:  # 'a' mode appends to the file
            #         np.savetxt(f, [np.append(obs, action)], delimiter=",", fmt="%.8f")
...
hidden_size = 256
num_players = 3
num_parallel_games = 4  # Adjust based on GPU capacity
save_expert_act_file = 'expert_act.csv'

def run_game_process(rank, model, device):
    # torch.cuda.set_device(device)  # Ensure each process uses the correct GPU
    
    agent = PPO(num_inputs, num_outputs, lr, agent_ids=list(range(num_players * num_parallel_games)), hidden_size=hidden_size, inference=True)
    agent.model.load_state_dict(model.state_dict())  # Load shared weights

    wr = test_agent(agent, [0, 1, 2], 100000)
    
    print(f"Winrate of the human agent over 100000 games against random agents: {wr}")
...
if __name__ == "__main__":
    ...
    shared_model = PPO(num_inputs, num_outputs, lr, agent_ids=list(range(num_parallel_games)), hidden_size=hidden_size, inference=True)
    shared_model.load_pretrained_imitation('pretrained.pth', '')
    shared_model = shared_model.model
    shared_model.share_memory()  # Make it accessible across processes
    run_game_process(0, shared_model, device)
